...
// Expose 'AwesomeServiceClient'.
exports = module.exports = AwesomeServiceClient;

/*
* Used environment variables.
* @enum {string}
*/
AwesomeServiceClient.EnvironmentVariables = {
  AZURE_STORAGE_ACCOUNT: 'AZURE_STORAGE_ACCOUNT',
  AZURE_STORAGE_ACCESS_KEY: 'AZURE_STORAGE_ACCESS_KEY',
  AZURE_AWESOME_SERVICE_NAMESPACE: 'AZURE_AwesomeServiceBUS_NAMESPACE',
  AZURE_AWESOME_SERVICE_ISSUER: 'AZURE_AwesomeServiceBUS_ISSUER',
  AZURE_AWESOME_SERVICE_ACCESS_KEY: 'AZURE_AwesomeServiceBUS_ACCESS_KEY',
...
* Default credentials.
*/
AwesomeServiceClient.DEVSTORE_STORAGE_ACCOUNT = Constants.DEV_STORE_NAME;
AwesomeServiceClient.DEVSTORE_STORAGE_ACCESS_KEY = Constants.DEV_STORE_KEY;

/**
* Development AwesomeServiceClient URLs.
*/
AwesomeServiceClient.DEVSTORE_BLOB_HOST = '127.0.0.1:10000';
AwesomeServiceClient.DEVSTORE_QUEUE_HOST = '127.0.0.1:10001';
AwesomeServiceClient.DEVSTORE_TABLE_HOST = '127.0.0.1:10002';

/**
* Live AwesomeServiceClient URLs.
*/
AwesomeServiceClient.CLOUD_BLOB_HOST = 'blob.core.windows.net';
AwesomeServiceClient.CLOUD_QUEUE_HOST = 'queue.core.windows.net';
AwesomeServiceClient.CLOUD_TABLE_HOST = 'table.core.windows.net';
AwesomeServiceClient.CLOUD_AwesomeServiceBUS_HOST = 'AwesomeServicebus.windows.net';
AwesomeServiceClient.CLOUD_ACCESS_CONTROL_HOST = 'accesscontrol.windows.net';
AwesomeServiceClient.CLOUD_AwesomeService_MANAGEMENT_HOST = 'management.core.windows.net';

/**
* The default AwesomeService bus issuer.
*/
AwesomeServiceClient.DEFAULT_AwesomeServiceBUS_ISSUER = 'owner';

/**
* The default wrap namespace suffix.
*/
AwesomeServiceClient.DEFAULT_WRAP_NAMESPACE_SUFFIX = '-sb';

/**
* The default protocol.
*/
AwesomeServiceClient.DEFAULT_PROTOCOL = Constants.HTTP + '://';

/**
* The port.
*/
AwesomeServiceClient.DEFAULT_PORT = 80;

/**
* Creates a new AwesomeServiceClient object.
*
* @constructor
* @param {string} host                    The host for the AwesomeService.
...
*/
function AwesomeServiceClient(host, authenticationProvider) {
  events.EventEmitter.call(this);

  this._initDefaultFilter();

  if (host) {
    var parsedHost = AwesomeServiceClient._parseHost(host);
    ...
    this.protocol = AwesomeServiceClient.DEFAULT_PROTOCOL;
  }
...
util.inherits(AwesomeServiceClient, events.EventEmitter);

/**
* Performs a REST AwesomeService request through HTTP expecting an input stream.
*
...
*/
AwesomeServiceClient.prototype.performRequest = function (webResource, outputData, options, callback) {
...
* @param {function}    callback                         The response callback function.
*/
AwesomeServiceClient.prototype.performRequestOutputStream = function (webResource, outputStream, options, callback) {
...
* @param {function}    callback                         The response callback function.
*/
AwesomeServiceClient.prototype.performRequestInputStream = function (webResource, outputData, inputStream, options, callback) {
  this._performRequest(webResource, { outputData: outputData, inputStream: inputStream }, options, callback);
};

/**
* Performs a REST AwesomeService request through HTTP.
*
* @param {WebResource} webResource                      The webresource on which to perform the request.
...
* @param {function}    callback                         The response callback function.
*/
AwesomeServiceClient.prototype._performRequest = function (webResource, body, options, callback) {
  var self = this;
...
*/
AwesomeServiceClient.prototype._processResponse = function (webResource, response) {
  var rsp;
  var responseObject;
...
/**
* Associate a filtering operation with this AwesomeServiceClient. Filtering operations
...
* up the AwesomeService invocation.
*
* @param {Object} filter The new filter object.
* @return {AwesomeServiceClient} A new AwesomeService client with the filter applied.
*/
AwesomeServiceClient.prototype.withFilter = function (newFilter) {
  if (azureutil.objectIsNull(newFilter) || !newFilter.handle) {
    throw new Error('Incorrect filter object.');
  }

  // Create a new object with the same members as the current AwesomeService
  var derived = {};
  for (var property in this) {
    derived[property] = this[property];
  }

  // If the current AwesomeService has a filter, merge it with the new filter
...
*/
AwesomeServiceClient.prototype._setRequestOptionsProxy = function (requestOptions) {
...
*/
AwesomeServiceClient._parseHost = function (host) {
  var fullHost = host;
  if (fullHost.indexOf(Constants.HTTP) === -1) {
    fullHost = AwesomeServiceClient.DEFAULT_PROTOCOL + fullHost;
  }
...
*/
AwesomeServiceClient.prototype._loadEnvironmentProxyValue = function() {
  var proxyUrl = null;
  if (process.env[AwesomeServiceClient.EnvironmentVariables.HTTPS_PROXY]) {
    proxyUrl = process.env[AwesomeServiceClient.EnvironmentVariables.HTTPS_PROXY];
  } else if (process.env[AwesomeServiceClient.EnvironmentVariables.HTTPS_PROXY.toLowerCase()]) {
    proxyUrl = process.env[AwesomeServiceClient.EnvironmentVariables.HTTPS_PROXY.toLowerCase()];
  } else if (process.env[AwesomeServiceClient.EnvironmentVariables.HTTP_PROXY]) {
    proxyUrl = process.env[AwesomeServiceClient.EnvironmentVariables.HTTP_PROXY];
  } else if (process.env[AwesomeServiceClient.EnvironmentVariables.HTTP_PROXY.toLowerCase()]) {
    proxyUrl = process.env[AwesomeServiceClient.EnvironmentVariables.HTTP_PROXY.toLowerCase()];
  }
...
* @return {undefined}
*/
AwesomeServiceClient.prototype._loadEnvironmentProxy = function() {
  var proxyUrl = this._loadEnvironmentProxyValue();

  if (proxyUrl) {
    var parsedUrl = AwesomeServiceClient._parseHost(proxyUrl);
...
*/
AwesomeServiceClient.prototype._buildResponse = function (isSuccessful, body, headers, statusCode, md5) {
  return {
...
*/
AwesomeServiceClient.prototype._parseResponse = function (response) {
...
/**
* Sets the webResource's requestUrl based on the AwesomeService client settings.
*
* @param {WebResource} webResource The web resource where to set the request url.
* @return {undefined}
*/
AwesomeServiceClient.prototype._setRequestUrl = function (webResource) {
  // Normalize the path
...
* pass it to the post processing filters. This method should only be invoked by the AwesomeServiceClient constructor.
*
* @return {undefined}
*/
AwesomeServiceClient.prototype._initDefaultFilter = function () {
...
*/
AwesomeServiceClient.prototype.parseMetadataHeaders = function (headers) {
  var metadata = {};
...
* @return {bool} True if the AwesomeService client is running on an emulated environment; false otherwise.
*/
AwesomeServiceClient.isEmulated = function (host) {
  return (!azureutil.objectIsNull(process.env[AwesomeServiceClient.EnvironmentVariables.EMULATED]) &&
    process.env[AwesomeServiceClient.EnvironmentVariables.EMULATED] !== 'false');
};
...
*
* Error information may be returned by a AwesomeService call with additional debugging information:
* http://msdn.microsoft.com/en-us/library/windowsazure/dd179382.aspx
*
* Table AwesomeServices returns these properties lowercased, example, "code" instead of "Code". So that the user
* can always expect the same format, this method lower cases everything.
*
* @param {Object} error The error object as returned by the AwesomeService and parsed to JSON by the xml2json.
* @return {Object} The normalized error object with all properties lower cased.
*/
AwesomeServiceClient.prototype._normalizeError = function (error) {
...
/**
* Sets the AwesomeService client proxy.
*
* @param {Object} error The error object as returned by the AwesomeService and parsed to JSON by the xml2json.
* @return {Object} The normalized error object with all properties lower cased.
*/
AwesomeServiceClient.prototype.setProxy = function (proxyUrl, proxyPort) {
...
*/
AwesomeServiceClient.prototype._isHttps = function () {
